...
public class StateMachineAutoConfiguration extends EnumStateMachineConfigurerAdapter<States, Events> {
    ...
    public void configure(StateMachineTransitionConfigurer<States, Events> transitions) throws Exception {
        transitions
                .withExternal()
                .source(States.STATE_INITIAL).target(States.STATE_AWAIT_ADD)
                .event(Events.EVENT_DO_ADD)
                .and()
                .withExternal()
                .source(States.STATE_INITIAL).target(States.STATE_AWAIT_MODIFY)
                .event(Events.EVENT_DO_MODIFY)
                .and()
                .withExternal()
                .source(States.STATE_INITIAL).target(States.STATE_AWAIT_DELETE)
                .event(Events.EVENT_DO_DELETE)
                .and()
                .withExternal()
                .source(States.STATE_PUBLISHED).target(States.STATE_AWAIT_ADD)
                .event(Events.EVENT_DO_ADD)
                .and()
                .withExternal()
                .source(States.STATE_PUBLISHED).target(States.STATE_AWAIT_MODIFY)
                .event(Events.EVENT_DO_MODIFY)
                .and()
                .withExternal()
                .source(States.STATE_PUBLISHED).target(States.STATE_AWAIT_DELETE)
                .event(Events.EVENT_DO_DELETE)
                .and()
                .withExternal()
                .source(States.STATE_AWAIT_ADD).target(States.STATE_PUBLISHED)
                .event(Events.EVENT_DO_PUBLISH)
                .and()
                .withExternal()
                .source(States.STATE_AWAIT_MODIFY).target(States.STATE_PUBLISHED)
                .event(Events.EVENT_DO_PUBLISH)
                .and()
                .withExternal()
                .source(States.STATE_AWAIT_DELETE).target(States.STATE_PUBLISHED)
                .event(Events.EVENT_DO_PUBLISH)
                .and()
                .withExternal()
                .source(States.STATE_AWAIT_ADD).target(States.STATE_INITIAL)
                .event(Events.EVENT_DO_ROLLBACK)
                .and()
                .withExternal()
                .source(States.STATE_AWAIT_MODIFY).target(States.STATE_INITIAL)
                .event(Events.EVENT_DO_ROLLBACK)
                .and()
                .withExternal()
                .source(States.STATE_AWAIT_DELETE).target(States.STATE_INITIAL)
                .event(Events.EVENT_DO_ROLLBACK)
                .and()
                .withExternal()
                .source(States.STATE_AWAIT_ADD).target(States.STATE_AWAIT_MODIFY)
                .event(Events.EVENT_DO_MODIFY)
                .and()
                .withExternal()
                .source(States.STATE_AWAIT_ADD).target(States.STATE_AWAIT_DELETE)
                .event(Events.EVENT_DO_DELETE)
                .and()
                .withExternal()
                .source(States.STATE_AWAIT_MODIFY).target(States.STATE_AWAIT_DELETE)
                .event(Events.EVENT_DO_DELETE)
                .and()
                .withExternal()
                .source(States.STATE_AWAIT_DELETE).target(States.STATE_AWAIT_MODIFY)
                .event(Events.EVENT_DO_MODIFY);
    }
